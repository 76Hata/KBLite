#!/usr/bin/env python3
"""Claude Code statusline script for KBLite (Windows).

Claude Code v2.1.80+ が stdin に渡す JSON から rate_limits を取得し:
  1. KBLite の data/sqlite/rate-limits.json に書き出し
  2. stdout に整形済み文字列を出力（ターミナルのステータスバー表示）
  3. Weekly バジェット超過時に推奨モデルを書き出し

セットアップ:
  Windows の Claude Code settings.json に以下を追加:
  {
    "statusLine": {
      "type": "command",
      "command": "python C:\\01_Develop\\project\\kblite\\scripts\\statusline.py"
    }
  }
"""
...
_OUT_PATH = Path(__file__).resolve().parent.parent / "data" / "sqlite" / "rate-limits.json"
