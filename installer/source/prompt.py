...
def build_team_prompt(message: str, agents: list, mode: str,
    ...
    if mode == "team-it":
        ...
        return prompt + rag_ctx + category_instruction + _TITLE_INSTRUCTION + _TASK_MANAGEMENT_INSTRUCTION + client_ctx
    else:
        parts = []
        ...
        prompt = "\n\n---\n\n".join(parts)
    return prompt + _IMAGE_INSTRUCTION + _DIAGRAM_INSTRUCTION + _DRAWIO_INSTRUCTION + rag_ctx + category_instruction + _TITLE_INSTRUCTION + _TURN_SUMMARY_INSTRUCTION + _KNOWLEDGE_EXTRACTION_INSTRUCTION + _TASK_MANAGEMENT_INSTRUCTION + client_ctx + lesson_context
